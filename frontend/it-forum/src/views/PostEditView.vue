...
const { draftSaved, draftSaving, checkAndRestore, clearDraft } =
  useDraft(auth.userId, postId, form)
...
onMounted(async () => {
  try {
    ...
    form.value = { title: post.title, content: post.content, categoryId: post.categoryId }
...
<template>
  <div class="edit-page" v-loading="pageLoading">
    <h2 class="page-title">编辑帖子</h2>

    <div class="card form-card">
      <div class="form-row">
        <el-select v-model="form.categoryId" placeholder="请选择版块" size="large" style="width:200px">
          <el-option v-for="c in categories" :key="c.id" :label="c.name" :value="c.id" />
        </el-select>
      </div>

      <div class="form-row">
        <input v-model="form.title" type="text" class="title-input" placeholder="请输入标题..." maxlength="200" />
      </div>

      <div class="form-row editor-row">
        <TiptapEditor v-model="form.content" placeholder="请输入内容..." @polish="handlePolish" />
      </div>

      <div class="form-actions">
        <div class="actions-left">
          <button class="polish-btn" :disabled="!form.content.trim() || polishLoading" @click="handlePolish(form.content)">
