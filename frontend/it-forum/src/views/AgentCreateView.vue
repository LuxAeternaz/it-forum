    ...
    <form class="agent-form glass-card" @submit.prevent="handleSubmit">
      ...
      <div class="form-group">
        ...
        <div class="input-prefix">
          ...
          <input
            ...
            type="text"
            class="form-input"
            placeholder="给你的智能体取个名字"
            maxlength="50"
          />
        </div>
        <p v-if="errors.name" class="form-error">{{ errors.name }}</p>
        <p v-else class="form-hint">只能包含中文、英文、数字和下划线，不能有空格。用户在评论中 @名称 即可召唤。</p>
      </div>
      ...
      <div class="form-group">
        ...
        <textarea
          ...
          class="form-textarea"
          rows="8"
          placeholder="描述智能体的性格、能力和回复风格..."
          maxlength="5000"
        />
        <p v-if="errors.persona" class="form-error">{{ errors.persona }}</p>
        <p v-else class="form-hint">
          示例：你是一个专注于前端开发的AI助手，擅长解答Vue、React等框架相关问题。你回答问题时简洁明了，喜欢用代码示例说明。
        </p>
      </div>
      ...
      <div class="form-group">
        ...
        <input
          ...
          maxlength="500"
        />
        <p class="form-hint">简短的介绍将展示在智能体卡片上。</p>
      </div>
...
.form-input {
  flex: 1; height: 40px; padding: 0 14px; border: none; outline: none;
...
.btn-cancel {
  padding: 10px 24px; border: 1px solid var(--color-border); border-radius: var(--radius-full);
